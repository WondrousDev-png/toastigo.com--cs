...
import { 
  ...
  Wifi, Zap, Activity, Thermometer, X, Loader
} from 'lucide-react';
...
const Order = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const firstAvailable = data.find(p => !p.outOfStock);
        if (firstAvailable) setSelectedColorId(firstAvailable.id);
        else if (data.length > 0) setSelectedColorId(data[0].id);
  ...
  const currentOption = products.find(o => o.id === selectedColorId) || products[0] || {};
  const currentPrice = currentOption.price || 0;

  // --- REAL PRINTER STATE ---
  const [printer, setPrinter] = useState({ online: false, temp: 0, state: "INITIALIZING", percent: 0 });
  ...
  const handleAddToCart = () => {
    if (!currentOption.id || currentOption.outOfStock) return;
  ...
  const handleColorSelect = (opt) => {
    if (!opt.outOfStock) setSelectedColorId(opt.id);
  };
  ...
  if (loadingProducts) {
      return (
          <div className={`min-h-screen ${THEME.bg} ${THEME.text} font-sans flex items-center justify-center`}>
    ...
    <div className={`min-h-screen ${THEME.bg} ${THEME.text} font-sans p-4 md:p-12 transition-colors duration-500`}>
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start pt-4 md:pt-10 pb-20">
        ...
        <div className="relative md:sticky md:top-24 space-y-6 order-1">
          <motion.div layoutId="product-card" className={`relative w-full bg-white rounded-[2rem] md:rounded-[3rem] border-4 ${THEME.border} shadow-2xl p-6 md:p-10 flex flex-col items-center justify-center overflow-hidden`}>
            
            {/* 🍞 3D BREAD SVG */}
            <motion.div animate={{ rotate: printer.state === "RUNNING" ? [0, 1, -1, 0] : 0, scale: printer.state === "RUNNING" ? [1, 1.02, 1] : 1 }} transition={{ repeat: Infinity, duration: 1 }} className="relative w-full max-w-[260px] md:max-w-[320px] aspect-square flex items-center justify-center">
              <svg viewBox="0 0 120 120" className="w-full h-full drop-shadow-2xl overflow-visible">
                <path d="M35,90 C25,90 22,80 22,70 L25,40 C25,30 20,25 35,15 C50,5 80,5 95,15 C110,25 105,30 105,40 L108,70 C108,80 105,90 95,90 Z" fill="#C68E56" stroke="black" strokeWidth="3" strokeLinejoin="round"/>
                <motion.path initial={{ fill: currentOption.hex }} animate={{ fill: currentOption.hex }} transition={{ duration: 0.3 }} stroke="black" strokeWidth="3" strokeLinejoin="round" d="M25,90 C15,90 12,80 12,70 L15,40 C15,30 10,25 25,15 C40,5 70,5 85,15 C100,25 95,30 95,40 L98,70 C98,80 95,90 85,90 Z"/>
            ...
            <AnimatePresence>
              {showSuccess && (
                <motion.div initial={{ opacity: 0, scale: 0.5, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.5 }} className="absolute inset-0 z-50 flex items-center justify-center bg-white/80 backdrop-blur-sm rounded-[2rem] md:rounded-[3rem]">
                  <div className={`p-6 rounded-3xl ${THEME.bg} border-4 ${THEME.border} shadow-2xl flex flex-col items-center mx-4`}>
                    ...
                    <p className="font-bold opacity-60 text-sm whitespace-nowrap">{quantity} x {currentOption.name}</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <p className="font-bold opacity-50 uppercase tracking-widest text-xs md:text-sm text-center">{currentOption.name}</p>

            <AnimatePresence>
              {customText && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className={`mt-4 px-4 py-2 rounded-lg border-2 ${THEME.border} bg-gray-50 font-mono text-sm uppercase break-all text-center max-w-full`}>
                  "{customText}"
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {printer.percent > 0 && (
              <div className="mt-4 space-y-1">
                ...
                <div className="w-full h-3 bg-black/10 rounded-full overflow-hidden border border-black/5">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${printer.percent}%` }} className={`h-full ${THEME.accent}`} />
                </div>
              </div>
            )}
          </div>
        ...
        <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="space-y-8 md:space-y-10 md:pl-8 order-2">
          <div>
            ...
            <div className="flex flex-wrap gap-4 gap-y-8">
              {products.map((opt) => (
                <button
                  ...
                  className={`group relative w-16 h-16 rounded-2xl border-4 transition-all duration-200 ring-4 ring-white/30
                ...
                >
                  {selectedColorId === opt.id && !opt.outOfStock && (
                    <div className={`absolute -top-3 -right-3 bg-white rounded-full p-1 border-2 ${THEME.border} shadow-sm z-10`}><Check size={12} strokeWidth={4} className="text-black" /></div>
                  )}
                  {opt.outOfStock && (
          ...
          <div className="space-y-4">
            <label className="font-black text-sm uppercase opacity-60 flex items-center gap-2"><Type size={16} /> Custom Engraving (Optional)</label>
            <input type="text" maxLength={15} value={customText} onChange={(e) => setCustomText(e.target.value)} placeholder="YOUR TEXT HERE" className={`w-full p-6 text-xl font-bold rounded-2xl border-4 outline-none transition-all placeholder:opacity-30 ${THEME.input} ${THEME.border}`} />
          ...
          <div className={`p-6 rounded-[2.5rem] bg-white/40 border-2 ${THEME.border} flex flex-col gap-6`}>
            <div className="flex flex-wrap justify-between items-center gap-4">
              ...
              <div className="text-right ml-auto">
                <div className="text-3xl font-black"><span className="text-lg opacity-50 mr-1">$</span>{(currentPrice * quantity).toFixed(2)}</div>
                <div className="text-xs font-bold opacity-50 uppercase">{quantity > 1 ? `$${currentPrice} each` : currentOption.name}</div>
              </div>
            </div>

            <button disabled={currentOption.outOfStock} onClick={handleAddToCart} className={`w-full py-5 rounded-2xl font-bold text-xl text-white shadow-lg active:scale-[0.98] transition-all flex items-center justify-center gap-3 ${THEME.accent} hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed`}>
              <ShoppingBag className="fill-white/20" /> {VALENTINE_MODE ? "Add Love to Cart" : "Add to Cart"}
            </button>
          </div>
